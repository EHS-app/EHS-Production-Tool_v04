...
- **Freelance Portal**: A second product surface served by the same artifact, mounted under `/portal/*` via Wouter. Targets external freelancers who log in with the same Clerk account; designed mobile-first with a sidebar on desktop and a bottom nav on mobile. Five screens — `Hub` (today + this week + month-to-date earnings + finish-profile prompt), `Gigs` (CRUD logbook with status pipeline `invited → confirmed → done → invoiced → paid`, one-tap status advance, optional flat fee that overrides hours×rate), `Availability` (month calendar with paint mode for `available`/`busy`, "all available", "all busy", "from today → state", and "clear month" shortcuts; flags conflicts where a confirmed/done/invoiced/paid gig overlaps a day marked available), `Earnings` (derived totals: this month / YTD / ready-to-invoice / outstanding / paid all-time, plus per-section gig tables and CSV export for the YTD set or the ready-to-invoice queue), and `Profile` (personal, dietary, language presets, insurance, invoicing details, and a free-text Skills tag editor with autocomplete from `portal/lib/skillLibrary.ts` covering disciplines, consoles, certifications, and languages). All portal data is stored in `localStorage` namespaced as `ehs-portal:${userId}` (per Clerk user) so freelancers can use it standalone without backend involvement. Code lives under `src/portal/` (entry: `Portal.tsx`, layout: `PortalLayout.tsx`, screens: `screens/*.tsx`, helpers: `lib/portalStorage.ts`, `lib/portalTheme.ts`, `lib/skillLibrary.ts`). The Production Tool header has a "Portal" link next to Sign out and the Portal header has a "Production Tool" link to switch back. Theme is shared via `localStorage` (the same `ehs-rigging-report-v2` key the Production Tool reads).
