...
- **LED Screen Report View**:
    ...
    - **Manufacturer datasheet per panel** — `InventoryItem` (and the derived `LedPanel` / `LedPanelSource`) accept optional rich-spec fields (`brand`, `model`, `pitchMm`, `avgPower`, `brightnessNits`, `refreshHz`, `viewingAngleH`/`V`, `voltage`, `ipRating`, `depthMm`, `moduleWMm`/`HMm`, `material`, `datasheetUrl`). Both **Uniview UR Pro 3.9** cabinets in the default inventory ship populated with the official spec sheet (3.91 mm pitch, 1000 nits, ≥3840 Hz refresh, 140°/140° viewing angles, AC 100–240 V, IP30, 75 mm deep, 250×250 mm modules, die-cast aluminium; max power 175 W / 350 W and typical power 58 W / 117 W per cabinet for the 500×500 / 1000×500 variants respectively). Each Screen row in the LED tab gets a small "ⓘ Specs" pill (rendered by `panelHasRichSpecs()` / `<DatasheetCard>`) that expands an in-row card with the full spec grid plus a link to the manufacturer product page. The LED dashboard's Power stat now also shows a `~X kW typical` sub-line whenever any chosen panel has `avgPower` set; `computeLedTotals()` falls back to `power / 3` for cabinets that don't.
