...
- **Stage Report View**: Nivtec deck calculator supporting standard deck sizes, leg heights, bracing notes, user-defined stages, "auto" and "manual" layout modes, load capacity calculation, optional handrails (four side checkboxes plus a custom-rail editor with two-click drawing — pick 1 m or 2 m, click two points on the SVG to drop a rail snapped to the 0.5 m grid, switch to Delete to remove individual rails, or Clear all). Manual mode supports deck rotation via right-click or Shift+click.
...
- **Drawing analyser (Rigg Plan tab)**: Producers can upload a stage / rigging drawing they made in any WYSIWYG tool and have it auto-fill the report tabs. The flow:
    - **Upload UI** lives at the top of the Rigg Plan tab (`<DrawingImporter>` inside `RiggPlanView`). Drag-and-drop or click-to-pick. Two file kinds are supported: **PDFs** up to ~8 MB binary (the typical export from Vectorworks / AutoCAD / Illustrator), and **images** (PNG / JPG / WebP / GIF) up to ~4.5 MB. The frontend size-checks before upload and renders a "PDF" badge placeholder for PDFs (browsers can't easily thumbnail multi-page PDFs in a small box) or an `<img>` thumbnail for images.
    - **Backend**: `POST /api/rigplan/analyze` (in `artifacts/api-server/src/routes/rigplanAnalyze.ts`) receives `{ fileDataUrl, context: { venue, projectName } }` (the legacy field name `imageDataUrl` is also accepted for backward compat). The data URL's MIME type decides whether the payload is sent to Anthropic Claude (`claude-sonnet-4-6`) as an `image` content block (for `image/*`) or a `document` content block (for `application/pdf`). The same strict JSON schema is requested either way: venue dims, stages, trusses (with point counts), lighting fixtures (each tagged with the truss it hangs on via a `trussName` field), LED screens (with either a panel grid OR a metric size in `widthM`/`heightM`) and sound items.
    - **Robust normalisation**: The route is hardened with a normaliser that coerces malformed AI output into a strict `ExtractedItems` shape and runs three correctness passes before returning data. (1) **Lighting dedupe** (`mergeLightingRows`): duplicate `(fixture-name, trussName)` pairs returned by the model — e.g. two MAC Aura clusters on the same LX bar — are folded into one row with summed qty; the merge key normalises whitespace/case and matches the client-side `trussKey` so the fixture is reliably wired to its system. (2) **Garbage-qty filter**: rows with qty > 200 (a model misread of part numbers or pixel counts as fixture counts) are dropped both pre- and post-merge so a "1000x VDO Fatron" never reaches the client. (3) **LED metres fallback** (`parseMetresFromNotes`): when the model leaves `widthM`/`heightM` null and only mentions the metric size in the screen's name or notes (e.g. "STØTE LED 5x3m Right", "IMAG- LED 7,5m x 4,5m"), a regex lifts the dimensions out — supports both dot- and comma-decimal European number formats, and ignores pixel counts ("768 x 1152 pixel") and panel counts ("16 x 9 panels") by requiring an "m" suffix. Zero/negative dimensions from the model are also normalised to null so the fallback can take over.
    - **Security on the analyser route**: `requireSignedIn` middleware (using `@clerk/express` `req.auth()`) rejects unauthenticated callers with 401 *before* the body is parsed, so the endpoint cannot be used as a free Anthropic relay. A simple in-memory token bucket (`recentByUser`) rate-limits each Clerk user to 6 analyses per minute and returns 429 with `Retry-After`. The 12 MB body parser is mounted route-scoped (`router.post(..., requireSignedIn, json({ limit: "12mb" }), rateLimit, handler)`); the global `express.json()` limit stays at the safe default (256 KB).
    - **Anthropic SDK** is configured via the Replit AI Integrations proxy env vars (`AI_INTEGRATIONS_ANTHROPIC_BASE_URL`, `AI_INTEGRATIONS_ANTHROPIC_API_KEY`). The `@anthropic-ai/sdk` package is a direct dependency of `@workspace/api-server`.
    - **Apply step**: The importer renders extracted items grouped by destination (Rigging / Lighting / LED / Stage / Sound) with per-item checkboxes. "Apply to reports" calls `applyExtractedItems(extracted, selection)` in `App.tsx`, which adds rows via the same factories the manual "+ New" buttons use (`makeSystem`, `makeShowFixture`, `newLedScreen`, `makeDefaultStage`, `makeSoundItem`) so the new items show up as plain editable rows in their respective tabs.
    - **Motor capacity auto-pick**: The schema captures an optional `hoistKg` per truss (clamped to {500, 1000} server-side). When present, `applyExtractedItems` sets `sys.hoistIndex = 1` for ≥750 kg hints and `0` otherwise — so a drawing labelled "1t" / "1000 kg" produces 1-tonne motors and "500 kg" stays at 500 kg. The DrawingImporter row surfaces the picked motor as "1 t motors" / "500 kg motors".
    - **Drawing as floor-plan backdrop**: The importer also exposes a "Use as floor plan" affordance. The selected file (image or rasterised PDF page 1, via `pdfjs-dist`) is stored separately in `localStorage` under `ehs-rigging-floor-plan-v1` (5 MB cap, soft-fails to memory-only), and rendered as an SVG `<image>` backdrop fitted to the venue rect in `RiggPlanView` with a "Clear" overlay button. The floor-plan helper `lib/floorPlan.ts` exports `loadFloorPlan` / `saveFloorPlan` / `fileToFloorPlan` / `pdfFirstPageToPng`. Worker is loaded via `import pdfWorkerSrc from "pdfjs-dist/build/pdf.worker.min.mjs?url"`.
- **Brief attachments (object storage)**:
    - **Object storage**: Provisioned via Replit App Storage. The api-server exposes `POST /api/storage/uploads/request-url` (Clerk-guarded, returns a presigned PUT URL + canonical `objectPath` of the form `/objects/<id>`) and `GET /api/storage/objects/*path` (streams the object back). The route module (`artifacts/api-server/src/routes/storage.ts`) hand-rolls a small validator (no api-zod dependency), enforces a 12 MB cap and an allow-list (PDF, PNG, JPEG, WebP, GIF), and is mounted route-scoped so the global JSON limit stays at the default.
    - **BriefAttachment**: `ProjectBrief` carries an `attachments: BriefAttachment[]` field (`{ id, name, contentType, sizeBytes, objectPath }`). `normalizeBrief` defensively coerces a missing `attachments` field to `[]` so older share URLs still decode. `objectPath` is validated against the canonical `/objects/...` shape so a tampered URL can't redirect downloads off-host.
    - **Producer flow**: `ShareBriefModal` reads the producer's stored floor plan via `loadFloorPlan()`, uploads it once via `uploadBriefAttachment` (a two-step flow: POST upload-URL with the Clerk JWT from `useAuth().getToken()`, then PUT bytes directly to the signed URL with no Authorization header), and embeds the resulting metadata in every brief. Upload failures are non-fatal — the textual brief still ships and a soft warning is shown to the producer.
    - **Freelancer flow**: `BriefDetail` renders a "Drawings & attachments" section when `brief.attachments.length > 0` with one row per attachment (filename, MIME-subtype, human-readable size) and a Download anchor pointing at `${BASE_URL}api/storage${objectPath}`.
