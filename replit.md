...
- **Crew Report View** (`components/CrewReportView.tsx`, data in `lib/crew.ts`) — simple call-sheet of crew members. Each row has a name, department (Rigging / Lighting / Video & LED / Stage / Sound / Other), call & off times (HH:MM, off-time before call-time wraps past midnight in `crewHours()`), day rate (€) and free-form notes. The dashboard shows headcount and cost per department plus three top-level totals (crew count, total person-hours, total cost in EUR). Crew list is persisted in `localStorage` under the `crew` field of the V2 blob and is normalized through `normalizeCrewMember()` on load so unknown / hand-edited values fall back to defaults instead of crashing. Money is EUR everywhere — the rest of the tool is Europe-only.
