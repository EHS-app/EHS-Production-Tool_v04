...
- **Drawing analyser (Rigg Plan tab)**: Producers can upload a stage / rigging drawing they made in any WYSIWYG tool and have it auto-fill the report tabs. The flow:
    ...
    - **Backend**: `POST /api/rigplan/analyze` (in `artifacts/api-server/src/routes/rigplanAnalyze.ts`) receives `{ fileDataUrl, context: { venue, projectName } }` (the legacy field name `imageDataUrl` is also accepted for backward compat). The data URL's MIME type decides whether the payload is sent to Anthropic Claude (`claude-sonnet-4-6`) as an `image` content block (for `image/*`) or a `document` content block (for `application/pdf`). The same strict JSON schema is requested either way: venue dims, stages, trusses (with point counts), lighting fixtures, LED screens and sound items. The route is hardened with a normaliser that coerces malformed AI output into a strict `ExtractedItems` shape.
