...
- **Drawing analyser (Rigg Plan tab)**: Producers can upload a stage / rigging drawing they made in any WYSIWYG tool and have it auto-fill the report tabs. The flow:
    - **Upload UI** lives at the top of the Rigg Plan tab (`<DrawingImporter>` inside `RiggPlanView`). Drag-and-drop or click-to-pick a PNG/JPG/WebP/GIF up to ~4 MB.
    - **Backend**: `POST /api/rigplan/analyze` (in `artifacts/api-server/src/routes/rigplanAnalyze.ts`) receives `{ imageDataUrl, context: { venue, projectName } }`, calls Anthropic Claude (`claude-sonnet-4-6`) with vision input, and asks for a strict JSON schema covering venue dims, stages, trusses (with point counts), lighting fixtures, LED screens and sound items. The route is hardened with a normaliser that coerces malformed AI output into a strict `ExtractedItems` shape.
