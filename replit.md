...
- **rigging-load-report** (`artifacts/rigging-load-report`) — EHS Rigging Load Report. A single-page React + Vite stage-tech tool with a top-level view switcher:
  ...
  - **LED Screen Report view** — pixel map generator (inspired by blinkingthings.com / led.fyi). Code lives in `src/lib/led.ts` (types + helpers + built-in panel library) and `src/components/LedScreenReportView.tsx` (UI + SVG canvas).
    - Built-in panel library: Uniview UR Pro 0.5×0.5m and 1×0.5m at 3.9mm and 2.9mm pitches, plus a "Custom panel…" entry where the user enters pixel/physical/weight/power values per screen.
    - Linked screens auto-derived from each system's `ledRows` whose inventory item is recognized as a Uniview LED panel (Molton fabric and unrecognized/custom items are skipped — those should be added manually on the LED tab). Per-screen layout/output/color/notes/customPanel stored in `ledLinkedMeta: Record<rigRowId, LedLinkedMeta>` and GC'd alongside linkedMeta. First-edit on a linked row seeds from the currently-resolved screen so the qty-derived default layout is preserved.
    ...
    - Dashboard totals: screens, panels, total pixels, area m², weight kg, power kW, and outputs needed (sum of `ceil(screen_pixels / portLimit)`, default 650,000 px/output, configurable). Settings normalized on load to prevent malformed persisted values.
