...
- **Help modal (Production Tool header)**: User-facing onboarding/reference modal opened by the "?" Help button at the start of the header doc-actions row. Component lives at `components/HelpModal.tsx` and walks through the overview, every header button, every tab, and four "good to know" tips. Bilingual (EN/NO) — every string is keyed under `header.help*` and `help.*` in the i18n catalog. Accessible: `role="dialog"`, focus trap (Tab/Shift+Tab cycle inside the dialog), focus return to the trigger on close, Escape / × / "Got it" / backdrop-click all close, and the keydown listener uses a stable `useCallback` wrapper so it does not churn between parent re-renders.
