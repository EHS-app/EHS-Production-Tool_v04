...
- **rigging-load-report** (`artifacts/rigging-load-report`) — EHS Rigging Load Report. A single-page React + Vite stage-tech tool with a top-level view switcher:
  ...
  - **Stage Report view** — Nivtec deck calculator (`src/lib/stage.ts` + `src/components/StageReportView.tsx`). User adds rectangular stages by W × D (snapped to 0.5 m); the greedy tiler in `tileStage()` works on a 0.5 m grid and tries each non-square deck (2×1, 0.5×2, 0.5×1) in BOTH catalog and rotated orientations, largest-area-first, so awkward dimensions like 1 × 0.5 m or 3.5 × 2 m fill cleanly. Catalog `StageDeckKey` is preserved across rotations so the breakdown table groups by physical deck type. **Legs** are computed as the union of unique deck-corner positions (so adjacent decks share corner legs); leg height is per-stage and chosen from the fixed set 20/40/60/80/100/120/140 cm. **Handrails** are optional per side (front/back/left/right); each enabled side picks 2 m pieces first then 1 m to fill, rounding up to the next metre for sub-metre sides. Stages persist under `PersistedV2.stages` and are normalized via `normalizeStage()` on load (defaults missing `rails`/legHeight/dimensions to safe values). `resetAll()` clears `stages` along with the other tab state. Top dashboard aggregates deck counts by key, leg counts, rail piece counts and total weight; per-stage card shows a top-down SVG with deck rectangles colour-coded by size, leg dots, and red rail strokes for visual sanity-check.
