...
- **Email-on-send (Resend / Outlook integration)**: T004 of the Crew Request slice 2 is intentionally deferred. The user dismissed both the Resend and Microsoft Outlook authorization flows. When picking this back up, propose an email integration again (or accept a pasted Resend API key as a `RESEND_API_KEY` secret) and wire `POST /api/portal/briefs` to send a Norwegian email per newly-inserted assignment. Track newly-inserted ids via `.returning()` on the existing `onConflictDoNothing` insert so resends do not duplicate. Subject pattern: `Ny forespørsel fra <producer name>`; link target: `${REPLIT_DOMAINS}/?view=portal&brief=<id>`; recipient lookup via `freelancer_profiles.email + full_name`; send fire-and-forget but log every attempt and failure.
