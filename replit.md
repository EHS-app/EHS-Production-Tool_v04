...
- **Client PDF Pack export**: A "Client Pack" button in the global header (`.header-doc-actions`, next to "Export Report" / "Share with Crew") opens a printable, single-document client-facing pack in a new tab via `exportClientPack()` (`src/lib/clientPackExport.ts`). The doc has 11 numbered sections — (1) Cover with EHS branding + event/client/venue/date/preparedBy meta, (2) Overview cards (crew, rigging systems, fixtures, distros, sound, stages, LED), (3) Schedule table per phase from `buildProjectSchedule()`, (4) Crew list with NOK day rates via `formatCrewDayRate`, (5) Rigging table per system (trusses, points, motor, static/dynamic, peak/SWL, util %, status pill) using `computeMetrics`, (6) Lighting v2.2 with per-distro feed/total W/worst leg/util/imbalance/phase pills using `computeDistroLoad`, (7) Sound table with category/qty/weight/power totals, (8) Stage table with dimensions/area/weight/capacity using `computeStage` + `computeStageTotals`, (9) LED screens with panel type/grid/cabinets/resolution + processor port load using `computeLedTotals`, (10) Risk Summary table aggregating SWL overloads, power overloads, phase imbalance, missing crew call times, and missing schedule dates with sorted 🔴 / 🟠 / 🟢 pills, and (11) Total Cost Summary by department from `computeCrewTotals`. Empty disciplines render literal "Not specified" rather than fabricated values. The popup is opened synchronously in the click handler (same pop-up-blocker pattern as the Power Plan and Stage Build Sheet exports) and the EHS logo is loaded via `getLogoDataUrl()` so the cover-page branding matches the other exports. The HTML is fully self-contained (inline CSS, A4 portrait print rules, sticky top-right Print / Close bar, orange `#f88000` cover frame and section headers) so the doc is portable to any browser print-to-PDF flow.
